...
and performs a Proportional Hypothesis Test on the Feed Efficiency
...
3. Sample Minimum
- Minimum number of samples to considered when implementing the
...
def calculate_proportions_hypothesis_test(n_one, p_one_total, n_two, p_two_total,
    ...
    # Flag Zero Denominator Results Otherwise Error Thrown
...
def analyze_ase_behavior(chicken_meta_dict, project, tissue,
                         multi_dim_adj_file_name, ase_group_sample_min):
    ...
    : Param ase_group_sample_min: Minimum number of samples that must be in each group to perform analysis
    ...
    output_file_name = 'hypothesis_testing_results.txt'
    ...
    output_file.write("Chrom\tPos\tID\tRef\tAlt\tTotal_Biallelic\tTotal_ASE\tASE_Alleles\tASE_Alleles_HFE\tASE_Alleles_LFE\t"
                      + "Sig_ASE_HFE\tTotal_Biallelic_HFE\tSig_ASE_LFE\tTotal_Biallelic_LFE\tHypothesis_Pvalue\n")
    ...
    non_testable_output_file.write("Chrom\tPos\tID\tRef\tAlt\tTotal_Biallelic\tTotal_ASE\tASE_Alleles\tASE_Alleles_HFE\tASE_Alleles_LFE\t"
                                   + "Sig_ASE_HFE\tTotal_Biallelic_HFE\tSig_ASE_LFE\tTotal_Biallelic_LFE\tFailure_Cause\n")
    ...
    for line in input_file:
        ...
        if line.startswith("#CHROM"):
            ...
            if (total_Biallelic_HFE < ase_group_sample_min) or (total_Biallelic_LFE < ase_group_sample_min) :
                ...
                non_testable_output_file.write(chromo + "\t" + position + "\t" + rs_id + "\t" + reference + "\t"
                                               ...
                                               + str(sig_ASE_LFE) + "\t" + str(total_Biallelic_LFE) + "\t" + "Fail- Group Sample Min (HFE_Biallelic="
            ...
            # Passing Data Analyze and Produce a P-value
                ...
                # Re-label variables for hypothesis testing
                group_one_result = sig_ASE_HFE
                group_one_total = total_Biallelic_HFE

                group_two_result = sig_ASE_LFE
                group_two_total = total_Biallelic_LFE
                ...
                p_value = calculate_proportions_hypothesis_test(group_one_result, group_one_total, group_two_result,
                                                                group_two_total, sideness, print_intermediate_steps)
                ...
                if np.isnan(p_value) == True:
                    ...
                    non_testable_output_file.write(chromo + "\t" + position + "\t" + rs_id + "\t" + reference + "\t"
                                                   ...
                                                   + ase_alleles_str + "\t" + ASE_HFE_alleles_str + "\t" + ASE_LFE_alleles_str + "\t"
                                                   + str(sig_ASE_HFE) + "\t" + str(total_Biallelic_HFE) + "\t"
                                                   + str(sig_ASE_LFE) + "\t" + str(total_Biallelic_LFE) + "\t" + "Fail- P-value is NaN" + "\n")
                    # Move Onto Next Line of Data
                    continue
                ...
                # Record all testable results (numeric p-values)
                    ...
                    output_file.write(chromo + "\t" + position + "\t" + rs_id + "\t" + reference + "\t"
                                      ...
                                      + ase_alleles_str + "\t" + ASE_HFE_alleles_str + "\t" + ASE_LFE_alleles_str + "\t"
                                      + str(sig_ASE_HFE) + "\t" + str(total_Biallelic_HFE) + "\t"
                                      + str(sig_ASE_LFE) + "\t" + str(total_Biallelic_LFE) + "\t" + str(p_value) + "\n")
...
def create_final_results_file(hypothesis_results_file_name, re_sorted_pvalues_list_dict, variant_info_dict):
    ...
    : Param hypothesis_results_file_name: Name of the results file
    ...
    # Output File Name
    output_file_name = "Final_Annot_Results_Prop_Hypo_Test.txt"
    ...
    output_file.write("Chrom\tPos\tID\tGene_Symbol\tConsequence\tImpact\tRef\tAlt\tTotal_Biallelic\tTotal_ASE\tASE_Alleles\tASE_Alleles_HFE\tASE_Alleles_LFE\t"
                      + "Sig_ASE_HFE\tTotal_Biallelic_HFE\tSig_ASE_LFE\tTotal_Biallelic_LFE\tHypothesis_Pvalue\tAdjusted_Pvalue\n")
...
def main():
    ...
    hypothesis_results = analyze_ase_behavior(chicken_meta_dict, project, tissue, multi_dim_adj_file_name, ase_group_sample_min)
    hypothesis_results_file_name = hypothesis_results[0]
    failure_results_file_name = hypothesis_results[1]

    # Extract P-values for Adjusting
    list_pvalues = extract_p_values_from_file(hypothesis_results_file_name)
    ...
    create_final_results_file(hypothesis_results_file_name, re_sorted_pvalues_list_dict, variant_info_dict)
